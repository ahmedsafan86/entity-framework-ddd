...
{
    public class DomainEventsDbContextTest
    {
        ...
        public async Task SaveChangesAsync_ShouldAddEventsToStore()
        {
            ...
            services.Replace(ServiceDescriptor.Scoped((_) => domainEventStoreMock.Object));
            ...
            {
                var dbContext = scope.ServiceProvider.GetRequiredService<DomainEventsDbContext>();
                var tasks = await dbContext.TrackingTasks
                    .Where(task => task.Status == Domain.TrackingTaskStatus.Ready)
                    .ToArrayAsync();
            ...
            {
                var dbContext = scope.ServiceProvider.GetRequiredService<DomainEventsDbContext>();
                var task = await dbContext.TrackingTasks
                    .FirstAsync(task => task.Status == Domain.TrackingTaskStatus.InProgress);
                task.Complete();
                await dbContext.SaveChangesAsync();
